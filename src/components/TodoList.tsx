...
const TodoList = () => {
	...
	}, [dispatch, editingId, editName, editImg, editYear, editAge]);

	const handleAdd = useCallback(() => {
		if (!name || !img || year === 0 || age === 0) {
			alert("Заполните все поля");
		} else {
			dispatch(addTodo({ name, img, year, age, checked: false }));
			setName("");
			setImg("");
			setYear(0);
			setAge(0);
		}
	}, [dispatch, name, img, year, age]);
...
export default React.memo(TodoList); // Исправлено для корректного использования React.memo
